...
# ✅ Setup: ensure users.csv is seeded if it doesn't exist
...
# ✅ Load and validate only non-empty passwords
df_users = pd.read_csv(USER_DB)
df_users = df_users.dropna(subset=["password"])
df_users = df_users[df_users["password"].astype(str).str.strip() != ""]
